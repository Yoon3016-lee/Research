/**
 * 이천시 민원만족도 — 나우앤서베이 RAW 응답 임포트
 * 사용: node scripts/import-icheon-raw-responses.mjs [raw-xml-path]
 */
...
const DEFAULT_RAW = join(__dirname, "..", "data", "imports", "icheon-satisfaction-raw.xml");

const SLUG = "이천시-민원만족도-a342b778";
...
function buildAnswers(row) {
  ...
  add(
    ...
    likertMultiAnswer(
      optsOf(1),
      [COL.q1, COL.q2, COL.q3, COL.q4, COL.q5, COL.q6, COL.q7, COL.q8],
      row,
    ),
  );

  const sat = cell(row, COL.satisfaction);
  add(2, mcAnswer(optsOf(2), sat));

  const complaint = cell(row, COL.complaint);
  const satN = Number(sat);
  if (complaint && Number.isInteger(satN) && satN >= 3) {
    add(3, { text: complaint });
  }

  const improve = cell(row, COL.improve);
  if (improve) add(4, { text: improve });
