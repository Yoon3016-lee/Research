/**
 * 2026년 청도군 민원서비스 고객 만족도 조사 — 나우앤서베이 RAW 응답 임포트
 * 사용: node scripts/import-cheongdo-raw-responses.mjs [raw-xml-path]
 */
...
const DEFAULT_RAW = join(
  ...
  "data",
  "imports",
  "cheongdo-customer-satisfaction-raw.xml",
);

const SLUG = "2026년-청도군-민원서비스-고객-만족도-조사-deb6b3f1";
...
function likertValue(raw, max = 5) {
  const v = Number(raw);
  if (!Number.isInteger(v) || v < 1 || v > max) return null;
  return { value: v };
}
...
function buildAnswers(row) {
...

  {
    ...
    const values = {};
    const likertCols = [COL.q1, COL.q2, COL.q3, COL.q4, COL.q5, COL.q6];
  ...
  add(2, mcAnswer(optsOf(2), q7));

  if (q7 === "1") {
    add(3, likertValue(cell(row, COL.q7_1)));
  } else if (q7 === "2") {
    const q8 = cell(row, COL.q8);
    add(4, mcAnswer(optsOf(4), q8));
    if (q8 === "1") {
      add(5, likertValue(cell(row, COL.q8_1)));
    }
  }

  add(6, likertValue(cell(row, COL.q9)));

  {
    const q10Idx = cell(row, COL.q10);
    const base = mcAnswer(optsOf(7), q10Idx);
    if (base) {
      const otherText = cell(row, COL.q10Other);
      const options = optsOf(7);
      const selected = options[Number(q10Idx) - 1];
      if (selected?.is_other && otherText) {
        add(7, { optionId: base.optionId, otherText });
      } else {
        add(7, base);
      }
    }
  ...
  add(8, mcAnswer(optsOf(8), cell(row, COL.age)));
  add(9, mcAnswer(optsOf(9), cell(row, COL.gender)));
