  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-zinc-500">
        ...
        분류표에서 단계별로 선택하세요. 코드가 애매하면 비정형 추론을 보조로 사용할 수 있습니다.
      </p>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-stretch">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          ...
          {!compact && onOpenRecommend ? (
            <button
              type="button"
              onClick={onOpenRecommend}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-300 bg-violet-600 px-4 py-2.5 text-base font-semibold text-white hover:bg-violet-700"
            >
              <AxiMark axiIconUrl={axiIconUrl} sizeClassName="h-9 w-9" />
              비정형 데이터 후보코드 추론
            </button>
          ) : null}
        </div>
